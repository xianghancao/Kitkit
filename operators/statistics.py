...
def skewness(x, period):
    ...
    for i in xrange(x.shape[1]):
        idx = np.isnan(x[:,i])
        res[:,i][~idx] = pd.rolling_skew(x[~idx][:,i], window=period)
    return res
...
def kurtosis(x, period):
    ...
    for i in xrange(x.shape[1]):
        idx = np.isnan(x[:,i])
        res[:,i][~idx] = pd.rolling_kurt(x[~idx][:,i], window=period)
    return res
...
def k_moment(x, period, pow=2):  
    ...
    for i in xrange(x.shape[1]):
        idx = np.isnan(x[:,i])
        res[:,i][~idx] = pd.rolling_apply(x[~idx][:,i], window=period, func=moment)
    return np.array(res)
